...
public class Orchestrator {
	...
	public JsonArray esegui (JsonObject questoJson, JsonArray risQueryPrec, Map<String, JsonObject> jsonUtili, Map<String, List<List<String>>> mappaWhere) throws Exception{
		System.out.println("IL NODO DA ESEGUIRE è: " + questoJson.get("table").getAsString());
		System.out.println("DI CUI CONOSCO: " + questoJson.toString());
		int contatore = 0;
		...
		if (tabelleDaEseguirePerPrima.isEmpty())
			System.out.println("DATO CHE è UNA FOGLIA, LA ESEGUO SUBITO"); 
		else
	...
	private JsonArray eseguiInParallelo(JsonObject questoJson, JsonArray risQueryPrec, Map<String, JsonObject> jsonUtili, Map<String, List<List<String>>> mappaWhere, List<String> tabelleDaEseguirePerPrima) throws Exception {
		...
		for (String tabellaDaEseguire : tabelleDaEseguirePerPrima){
			...
			System.out.println("RISULTATO FIGLIO :\n" + risultato.toString());
		...
		for (int i=0; i<jsonArray1.size();i++){
			...
			for (int j=0; j<jsonArray2.size();j++){
				...
				if (primaryKey1.equals(primaryKey2)){
					JsonObject rigaUnita = concatenate(rigaRis1, rigaRis2);
					tuttiIRisultati.add(rigaUnita);
				}
			}
		}
		System.out.println("risultati figli (stella) uniti: \n" + tuttiIRisultati.toString());
		return tuttiIRisultati;
	}


	private JsonObject concatenate(JsonObject rigaRis1, JsonObject rigaRis2) {
		JsonObject merge = new JsonObject();
		Set<Map.Entry<String, JsonElement>> entriesRigaRis1 = rigaRis1.entrySet();//ritorna i membri del jsonObject
		for (Map.Entry<String, JsonElement> entry: entriesRigaRis1) {
			String chiave = entry.getKey();
			merge.addProperty(chiave, rigaRis1.get(chiave).getAsString());
		}
		Set<Map.Entry<String, JsonElement>> entriesRigaRis2 = rigaRis2.entrySet();
		for (Map.Entry<String, JsonElement> entry: entriesRigaRis2) {
			String chiave = entry.getKey();
			merge.addProperty(chiave, rigaRis2.get(chiave).getAsString());
		}
		return merge;
	}
